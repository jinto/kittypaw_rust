...
- [ ] kittypaw-store migration 005: execution_history + user_context 테이블
- [ ] schedule.rs `run_skill()` 후 실행 기록 삽입
- [ ] 패턴 매칭: 같은 파라미터 3회 → 기본값 자동 적용
- [ ] 패턴 매칭: 실패 시 자동 재시도 (exponential backoff)
- [ ] 패턴 매칭: 시간 패턴 감지 → 스케줄 제안
- [ ] 프라이버시: 민감 데이터 필터링, 30일 자동 삭제

### 🔴 P0: GUI 대시보드 퍼스트
> 메인 화면은 "내 자동화 현황판" — 채팅이 아니다
- [ ] `dashboard.rs` 신규 컴포넌트 (활성 스킬 + 실행 결과 + 다음 스케줄)
- [ ] 오늘의 실행 요약 (성공/실패/자동최적화)
- [ ] "조용한 개선" 카운터: "이번 주 N번의 자동 최적화 적용됨"
- [ ] 채팅을 세 번째 탭 또는 사이드 패널로 이동 (설정/디버깅 전용)
- [ ] 앱 실행 시 대시보드가 기본 화면
