...
fn config(timeout_secs: u64, memory_limit_mb: u64) -> SandboxConfig {
    SandboxConfig {
        timeout_secs,
        memory_limit_mb,
        allowed_paths: vec![],
...
async fn test_fs_traversal_blocked() {
    ...
    let s = Sandbox::new(config(10, 128));
    let r = s
...
async fn test_cpu_spin_timeout() {
    let s = Sandbox::new(config(2, 128));
